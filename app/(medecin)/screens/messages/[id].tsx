...
export default function MedecinConversationScreen() {
  ...
  const pickAndSendImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        await apiService.sendMessage(String(id), 'Image');
      }
    } catch (error) {
  ...
  const pickAndSendFile = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        await apiService.sendMessage(String(id), asset.name || 'Fichier');
      }
    } catch (error) {
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    if (displayUrl && displayUrl.startsWith('/')) displayUrl = `${API_BASE_URL}${displayUrl}`;
